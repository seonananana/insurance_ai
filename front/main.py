...
/* 전체 배경 */
.stApp { background: linear-gradient(180deg, #E3F2FD 0%, #FFFFFF 100%); }
...
/* 히어로 헤더 */
.hero-header { background: linear-gradient(135deg, #1E88E5 0%, #1565C0 100%); padding: 3rem 2rem; border-radius: 16px; margin-bottom: 2rem; box-shadow: var(--shadow-lg); position: relative; overflow: hidden; }
.hero-header::before { content: ''; position: absolute; top: -50%; right: -20%; width: 400px; height: 400px; background: rgba(255, 255, 255, 0.1); border-radius: 50%; }
...
/* 홈 카드/배지/통계 */
.stat-card { background: #fff; border-radius: 16px; padding: 1.5rem; box-shadow: var(--shadow-sm); border: 1px solid var(--border-light); text-align: center; }
.stat-value { font-size: 2rem; font-weight: 900; color: var(--primary-blue); margin-bottom: .25rem; }
.stat-label { font-size: .875rem; color: var(--text-secondary); font-weight: 500; }
.badge { display: inline-block; padding: .25rem .75rem; border-radius: 20px; font-size: .75rem; font-weight: 600; background: #E3F2FD; color: var(--primary-blue); }
.badge.success { background: #81C784; color: #fff; }
.badge.error { background: #E57373; color: #fff; }

.insurer-card { background: #fff; border: 2px solid var(--border-light); border-radius: 12px; padding: 1.5rem; margin: 1rem 0; cursor: pointer; transition: all .3s ease; box-shadow: var(--shadow-sm); }
.insurer-card:hover { border-color: var(--primary-blue); box-shadow: var(--shadow-md); transform: translateY(-2px); }
.insurer-card.selected { background: linear-gradient(135deg, #E3F2FD 0%, #BBDEFB 100%); border-color: var(--primary-blue); box-shadow: var(--shadow-md); }

.home-feature-card { background: #fff; border-radius: 16px; padding: 2rem; box-shadow: var(--shadow-md); border: 2px solid var(--border-light); transition: all .3s ease; height: 100%; }
.home-feature-card:hover { border-color: var(--primary-blue); transform: translateY(-4px); box-shadow: var(--shadow-lg); }
...
def _normalize_references(resp_json: dict):
    ...
    if isinstance(resp_json.get("references"), list):
        for it in resp_json["references"]:
            ...
            page = it.get("page") if it.get("page") is not None else it.get("page_no")
            score = it.get("score")
            snippet = it.get("content") or it.get("text") or it.get("snippet") or ""
            title = f"{fname} (p.{page})" if page is not None else fname
...
# 상단 네비게이션 바 (수평 정렬 고정)
# ─────────────────────────────────────────────────────────────
nav_col1, nav_col2 = st.columns([2, 3], gap="large")

with nav_col1:
    st.markdown('<div class="nav-title-button nav-row">', unsafe_allow_html=True)
...
# Settings 페이지
    ...
    with col2:
        ...
        st.markdown("### 💬 대화 관리")
        st.info(f"현재 대화 메시지 수: {len(_msgs())}개")
...
# Q&A 상담 페이지
    ...
    if user_text:
        ...
        with st.spinner("🔍 관련 문서를 검색하고 답변을 생성하고 있습니다..."):
            ...
            log.append({"role": "assistant", "content": answer, "refs": refs})

            # 자동 PDF: 동일 응답 반복 다운로드 방지 가드
            answer_id = hash((ss.insurer, user_text, answer))
            if ss.auto_pdf and answer_id not in ss.pdf_guard:
                ss.pdf_guard.add(answer_id)
...
# PDF 리포트 페이지
    ...
    with st.form("pdf_form"):
        ...
        with col3:
            required_docs = st.text_area("📋 필요 서류", value="진단서\n진료비 영수증\n입퇴원확인서", height=120)
        with col4:
            timeline = st.text_area("📅 타임라인", placeholder="2025-01-02 최초 내원\n2025-01-05 입원\n2025-01-10 퇴원", height=120)
